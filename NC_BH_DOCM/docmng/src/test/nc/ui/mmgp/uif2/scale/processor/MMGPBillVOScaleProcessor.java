	...
	protected static class BillVoFieldValueGetter implements IFieldValueGetter {
		...
		public Object getValue(FieldInfo fieldInfo) {
			if (PosEnum.valueOf(fieldInfo.getPos()) == PosEnum.body) {
				this.getChildVo().getAttributeValue(fieldInfo.getItemkey());
			} else {
				return this.getParentVo().getAttributeValue(
						fieldInfo.getItemkey());
			}
			return null;
		}
		...
		private CircularlyAccessibleValueObject[] getVOsByClazzFromVO(IBill bill) {
			...
			try {
				...
				if (childrens.length == 1) {
					...
					if(attrbute == null) {
						throw new IllegalArgumentException("表头VO元数据没有属性:"
								+ getTabCode());
					}
...
//					}
				...
				if (voMeta == null) {
					throw new IllegalArgumentException("没有tabcode:"
							+ getTabCode());
				}
