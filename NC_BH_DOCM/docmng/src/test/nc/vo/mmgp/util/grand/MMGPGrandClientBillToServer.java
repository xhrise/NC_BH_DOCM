...
public class MMGPGrandClientBillToServer<E extends IBill> {
    ...
    private ISuperVO[] compareChildren(ISuperVO[] originVOs,
        ...
        if (originVOs == null) {
            ...
            this.setVOStatus(originVOs, VOStatus.DELETED);
            return originVOs;
        } else {
    ...
    private void createGCDiffVO(ISuperVO originVO,
        ...
        for (IAttribute attrTemp : childAttr) {
            ISuperVO[] oldChildVOs = (ISuperVO[]) attrTemp.getAccessStrategy().getValue(originVO, attrTemp);
            ISuperVO[] childVOs = (ISuperVO[]) attrTemp.getAccessStrategy().getValue(vo, attrTemp);
