...
public class MMGPCmnQueryService implements IMMGPCmnQueryService {
     ...
     */
    @Override
    public Object[] cmnQueryDatasByCondition(Class< ? > clazz,
     ...
     */
    private Object[] queryDatasByConditionInner(Class< ? > clazz,
                                                String con) throws MetaDataException {
     ...
     * {@inheritDoc}
     */
    public IBill cmnQueryChildrenByParentPK(Class< ? > clazz,
        ...
        try {
            ...
            IBill aggVO =
                    (IBill) queryService.queryBillOfNCObjectByPKWithDR(clazz, parentPk, true).getContainmentObject();
     ...
     */
    @Override
    public Map<String, Map<Class< ? extends ISuperVO>, SuperVO[]>> cmnQueryChildrenByParent(Map<BillLazilyLoaderVO, List<Class< ? extends ISuperVO>>> map)
            throws BusinessException {

        Map<String, Map<Class< ? extends ISuperVO>, SuperVO[]>> resultMap =
                new HashMap<String, Map<Class< ? extends ISuperVO>, SuperVO[]>>();
        try {
            for (Entry<BillLazilyLoaderVO, List<Class< ? extends ISuperVO>>> entry : map.entrySet()) {
                BillLazilyLoaderVO loaderVO = entry.getKey();

                Class< ? extends IBill> billClz = loaderVO.getBillClass();
                String parentID = loaderVO.getPk();
                Map<Class< ? extends ISuperVO>, SuperVO[]> billMap =
                        new HashMap<Class< ? extends ISuperVO>, SuperVO[]>();
                IBill aggVO = this.cmnQueryChildrenByParentPK(billClz, parentID);
                for (Class< ? extends ISuperVO> childClazz : entry.getValue()) {
                    billMap.put(childClazz, (SuperVO[]) aggVO.getChildren(childClazz));
                }
    ...
    private Object[] collectionToArray(Collection objs,
                                       Class< ? > clazz) {
    ...
    private Collection<Object> queryObjects(Class< ? > clazz,
                                            String con,
        ...
        List<Object> results =
                (List<Object>) queryService.queryBillOfVOByCondWithOrder(
                    clazz,
                    con,
                    true,
                    bLazyLoad,
        ...
        if (bLazyLoad && MMCollectionUtil.isNotEmpty(results)) {
            ...
            if (MMStringUtil.isNotEmpty(pk)) {
                NCObject firstNCObj = queryService.queryBillOfNCObjectByPKWithDR(clazz, pk, true);
    ...
    public Object[] cmnQueryDatasByConditionWithDataPermission(Class< ? > clazz,
                                                               String con,
                                                               String resouceCode,
                                                               String operatecode) throws BusinessException {
        try {
            ...
            String powerSql = getPowerSql(tableName + "." + pkField, resouceCode, operatecode);
            String newCondition;
     ...
     */
    private String getPowerSql(String joinColumn,
                               String resouceCode,
                               String operatecode) {
        ...
        try {
            ...
            IDataPermissionPubService queryService = NCLocator.getInstance().lookup(IDataPermissionPubService.class);
            String sqlWherePart =
                    queryService.getDataPermissionSQLWherePart(pk_user, resouceCode, operatecode, pk_group);
            // try {
     ...
     */
    @Override
    public Object cmnQueryDatasByPk(Class< ? > clazz,
                                    String pk) throws BusinessException {
        try {
            IMDPersistenceQueryService queryService =
                    NCLocator.getInstance().lookup(IMDPersistenceQueryService.class);
            NCObject ncObj = queryService.queryBillOfNCObjectByPKWithDR(clazz, pk, true);
    ...
    public Object[] cmnQueryDatasByScheme(Class< ? extends AbstractBill> clazz,
        ...
        try {
            ...
            // 查询结果
            AbstractBill[] vos = (AbstractBill[]) new BillLazyQuery(clazz).query(queryScheme, orderByString);

            // 过滤状态
            return MMGPApproveFilter.filterForApprove(queryScheme, vos, billTypeCode);
        } catch (Exception e) {
            ExceptionUtils.marsh(e);
        }
        return new Object[0];
    ...
    protected String[] getOrderPaths(Class< ? > clazz) throws MetaDataException {
        ...
        String pkOrg = MMMetaUtils.getBizInterfaceMapInfo(IBDObject.class.getName(), bean, MMGlobalConst.PK_ORG);
    ...
    /*
     * (non-Javadoc)
     * @see nc.itf.mmgp.uif2.IMMGPCmnQueryService#cmnQueryDatasByPks(java.lang.Class, java.lang.String[])
     */
    @SuppressWarnings("rawtypes")
    @Override
    public Object[] cmnQueryDatasByPks(Class< ? > clazz,
                                       String[] pks) throws BusinessException {
        try {
            IBean bean = MDBaseQueryFacade.getInstance().getBeanByFullClassName(clazz.getName());
            String pkName = bean.getTable().getName() + "." + bean.getTable().getPrimaryKeyName();
            ...
            Collection result =
                    NCLocator
                        ...
                        .queryBillOfVOByCondWithOrder(clazz, sql, true, false, this.getOrderPaths(clazz));
